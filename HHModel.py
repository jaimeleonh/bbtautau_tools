...
# xxHHFormula -> implements the matrix component representation,
#                that calculates the symbolic scalings
...
from sympy import Matrix, Pow, Mul, symbols
...
class VBFHHFormula:
    ...
    def calculatecoeffients(self):
        ...
        CV, C2V, kl, a, b, c, iab, iac, ibc, s1, s2, s3, s4, s5, s6 =\
            symbols('CV C2V kl a b c iab iac ibc s1 s2 s3 s4 s5 s6')
...
class HHModel(PhysicsModel):
    ...
    def check_validity_ggf(self, ggf_sample_list):
        if len(ggf_sample_list) != 3:
            raise RuntimeError("%s : malformed GGF input sample list "
                "- expect 3 samples" % self.name)
        if not isinstance(ggf_sample_list, list) and not isinstance(ggf_sample_list, tuple):
            raise RuntimeError("%s : malformed GGF input sample list "
                "- expect list or tuple" % self.name)
        for s in ggf_sample_list:
            if not isinstance(s, GGFHHSample):
                raise RuntimeError("%s : malformed GGF input sample list "
                    "- each element must be a GGFHHSample" % self.name)

    def check_validity_vbf(self, vbf_sample_list):
        if len(vbf_sample_list) != 6:
            raise RuntimeError("%s : malformed VBF input sample list "
                "- expect 6 samples" % self.name)
        ...
        for s in vbf_sample_list:
            if not isinstance(s, VBFHHSample):
                raise RuntimeError("%s : malformed VBF input sample list "
                    "- each element must be a VBFHHSample" % self.name)

    def dump_inputs(self):
        print "[INFO]  HH model : ", self.name
        print "......  GGF configuration"
        for i, s in enumerate(self.ggf_formula.sample_list):
            print "        {0:<3} ... kl : {1:<3}, kt : {2:<3},".format(i, s.val_kl, s.val_kt,),
            print "xs : {0:<3.8f} pb, label : {1}".format(s.val_xs, s.label)
        print "......  VBF configuration"
        for i, s in enumerate(self.vbf_formula.sample_list):
            print "        {0:<3} ... CV : {1:<3}, C2V : {2:<3},".format(i, s.val_CV, s.val_C2V),
            print "kl : {0:<3}, xs : {1:<3.8f} pb, label : {2}".format(s.val_kl, s.val_xs, s.label)
    ...
    def getYieldScale(self, bin, process):
        ...
        if process not in self.scalingMap:
        ...
        if len(imatched_ggf) + len(imatched_vbf) != 1:
            print "[ERROR] : in HH model named", self.name, "there are",
            print len(imatched_ggf), "GGF name matches and", len(imatched_vbf), "VBF name matches"
            raise RuntimeError("HHModel : could not uniquely match the process"
                " %s to the expected sample list" % process)
    ...
    def done(self):
        ...
        for k, i in self.scalingMap.items():  # key -> process, item -> [(isample, 'type')]
            ...
            for s in samples:
                if s not in scalings:
                    scalings[s] = []
                scalings[s].append(k)

        for key, val in scalings.items():
            if len(val) > 1:
                print "[ERROR] : in HH model named", self.name,
                print "there is a double assignment of a scaling : ", key, " ==> ", val
        ...
        if n_GGF > 0 and n_GGF != 3:
            raise RuntimeError("HHModel : you did not pass all the "
                "3 samples needed to build the GGF HH model")

        if n_VBF > 0 and n_VBF != 6:
            raise RuntimeError("HHModel : you did not pass all the "
                "6 samples needed to build the VBF HH model")
